...
def arm(target, rung, sub=2, shuffle=0):
    X, y, s = load(target)
    if shuffle:
...
TG = _s.argv[1]; RG = int(_s.argv[2]); LO = int(_s.argv[3]); HI = int(_s.argv[4])
for k in range(max(LO, 1), HI):
    B, A = arm(TG, RG, shuffle=k)
    av = [v for v in A.values() if v is not None]
    if not av: continue
    _j.dump({"target": TG, "rung": RG, "seed": k,
             "auc": st.mean(av), "bal": st.mean(list(B.values())),
             "per_subject_auc": A},
            open(f"{T}/results/aucnull_r{RG}_{TG}_{k}.json", "w"), indent=1)
    print(f"perm {k}: AUC {st.mean(av):.4f} bal {st.mean(list(B.values())):.4f}", flush=True)
